...
4. Fill in the inputs:
   ...
   - **createAzureOpenAiResource** (optional, default **false**): When **true**, deploy a per-POC Azure OpenAI account, **OpenAIApiKey** in Key Vault, and model deployments. When **false** (default), use shared OpenAI via **`AZURE_OPENAI_ACCOUNT_NAME`** + **`AZURE_OPENAI_ACCOUNT_SECRET`** for backend **`OPENAI_ACCOUNT_EUS2`** when both secrets are set; otherwise that app setting is omitted. Blob storage is still restricted to Web App outbound IPs after App Services deploy; this workflow does not change Azure OpenAI firewall rules.
