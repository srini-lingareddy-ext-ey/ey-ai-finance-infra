...
4. Fill in the inputs:
   ...
   - **appChoice** (optional): App variant for init.sql; chooses `resources/<appChoice>/db/init.sql` from the ey-ai-finance repo (`aifinance-next` or `aifinance`; default `aifinance-next`).
...
| 11   | **Create blob payload files:** connection string from **`scripts/storage_account_connection_string.sh`** (matches portal); probes the storage **data plane** (retries; **300s** sleep budget shared with upload retries), then runs **`scripts/seed_blob_payloads.py`**: **`configs/payloads/*.json`** → minimal **`{}`** as **`application/json`**; **`configs/lighthouse/*.yml`** and **`configs/chat/*.yml`** → file contents from **`bicep/configs/`** (e.g. **`lh.yml`**, **`chat.yml`**). Tenant folder in the manifest uses the **`"{pocSlug}"`** key, substituted with the workflow **`pocSlug`**. |
| 12   | Syncs App Configuration from `bicep/configs/backend_configs.yml` (label **`tenant-1`**).                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
Set `keyVaultName` and secure **`appConfigConnectionString`** (App Configuration **Primary** read-write connection string: `Endpoint=...;Id=...;Secret=...`) in the parameters file or override on the command line. Use **`scripts/app_configuration_connection_string.sh <store> <resource-group>`** (same as **Deploy POC**), or copy from the portal (**Access keys** → **Primary**). Do not rely on **`az deployment sub show`** for **`appConfigConnectionString`**: **`@secure()`** outputs are omitted from deployment GET responses. Do not commit real secrets. To mirror the workflow’s backend Postgres env vars, pass **`postgresHost`**, **`postgresDatabaseName`**, **`postgresUser`**, **`postgresPort`**, and secure **`postgresPassword`** (use a Key Vault parameters reference or `--parameters postgresPassword=...` from a secure input—do not commit secrets).
...
| appConfigConnectionString                                      | Yes      | Secure. **Primary** read-write connection string. **`scripts/app_configuration_connection_string.sh`**, or portal **Access keys** → **Primary**. (Bicep can **declare** output **`appConfigConnectionString`** for consistency; Azure does not return **`@secure()`** output **values** from **`az deployment show`**.)                                                                                                                      |
...
- **appConfigEndpoint**, **appConfigStoreName**, **appConfigConnectionString** (secure) — App Configuration. The connection string output is for template wiring; **retrieve the value** for phase 3 with **`scripts/app_configuration_connection_string.sh`** or the portal (**Primary**), not **`az deployment show`**. **`appConfigEndpoint`** is the HTTPS endpoint only.
...
- **App services only:** Use **appservices-stack.bicep** as in phase 3, with `keyVaultName` and secure **`appConfigConnectionString`** from **`scripts/app_configuration_connection_string.sh`** or the portal (**Primary** key). Do not use **`az deployment show`** for secure **`appConfigConnectionString`**.
