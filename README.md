...
Set `keyVaultName` and secure **`appConfigConnectionString`** (App Configuration **Primary** read-write connection string: `Endpoint=...;Id=...;Secret=...`) in the parameters file or override on the command line. Use phase 1 output **`appConfigConnectionString`** (e.g. **`az deployment sub show --name <deployment> --query "properties.outputs.appConfigConnectionString.value" -o tsv`**), or copy from the portal (**Access keys** → **Primary**). Do not commit real secrets. To mirror the workflow’s backend Postgres env vars, pass **`postgresHost`**, **`postgresDatabaseName`**, **`postgresUser`**, **`postgresPort`**, and secure **`postgresPassword`** (use a Key Vault parameters reference or `--parameters postgresPassword=...` from a secure input—do not commit secrets).
...
| appConfigConnectionString                                      | Yes      | Secure. **Primary** read-write connection string. Phase 1 output **`appConfigConnectionString`** (same value Bicep builds from **Primary** key), or portal **Access keys** → **Primary**.                                                                                                                      |
...
- **appConfigEndpoint**, **appConfigStoreName**, **appConfigConnectionString** (secure) — App Configuration. Pass **`appConfigConnectionString`** (**Primary** read-write connection string) to phase 3; **`appConfigEndpoint`** is the HTTPS endpoint only.
...
- **App services only:** Use **appservices-stack.bicep** as in phase 3, with `keyVaultName` and secure **`appConfigConnectionString`** from the existing core deployment output **`appConfigConnectionString`** (or portal **Primary** key).
