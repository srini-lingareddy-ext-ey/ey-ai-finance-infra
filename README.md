...
| 14   | Syncs blob path references (**payloads**, **lighthouse**, **chat** from `blob_payloads.json`) into App Configuration via `scripts/sync_blob_payload_refs_to_appconfig.py` (keys = path under `configs/` with `:` separators; values = blob names under the `tenants` container).                                 |
