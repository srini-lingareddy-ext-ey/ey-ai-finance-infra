...
| 9    | **Populate MongoDB:** runs **`scripts/seed_mongo_eyaifin.py`** — ensures database **`eyaifin`** and collection **`chats`** with a bootstrap document (**`messages`: []**) when the collection is empty (idempotent on re-runs). URI matches Key Vault **`MongoConnectionString`** (including **`authSource=admin`**).                                                                                                                                                                                                                                                                                     |
